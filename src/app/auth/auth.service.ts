...
export interface CurrentUser { id: string; email: string; name: string; role: 'admin'|'operator'|'viewer'; }
...
export class AuthService {
  ...
  get accessToken(): string | null { return localStorage.getItem(this.ACCESS_KEY); }
  ...
  login(email: string, password: string): Observable<AuthTokens> {
    return this.http.post<AuthTokens>(`${environment.apiUrl}/auth/login`, { email, password })
      .pipe(tap((tokens: AuthTokens) => this.storeTokens(tokens)));
  }

  refresh(): Observable<AuthTokens> {
    const refreshToken = localStorage.getItem(this.REFRESH_KEY);
    return this.http.post<AuthTokens>(`${environment.apiUrl}/auth/refresh`, { refreshToken })
      .pipe(tap((tokens: AuthTokens) => this.storeTokens(tokens)));
  }
  ...
  me(): Observable<CurrentUser> {
    return this.http.get<CurrentUser>(`${environment.apiUrl}/auth/me`)
      .pipe(tap((user: CurrentUser) => this.currentUser$.next(user)));
  }
  ...
  private tryLoadUser(): void {
    if (this.accessToken) { this.me().subscribe({ error: () => this.clearTokens() }); }
  }
}
